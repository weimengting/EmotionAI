...
from former_DFER.video_dataset import VideoDataset
...
model = GenerateModel(num_classes=2).to(device)  # 使用定义的模型，并将模型移动到GPU


dict_s = torch.load('trained_former_DFERmodel_val' + str(index) + '.pth')
