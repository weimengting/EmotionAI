...
%  Pay attention to "TInterval". "TInterval * 2 + 1" should be smaller than the length of the input sequence "Length". For example, 
%  if one sequence includes seven frames, and you set TInterval to three, only the pixels in the frame 4 would be considered as central 
        ...
        for yc = BorderLength + 1 : height - BorderLength
            ...
            for xc = BorderLength + 1 : width - BorderLength
                ...
                for p = 0 : XYNeighborPoints - 1
                    ...
                    CurrentVal = VolData(lty, ltx, i) * (1 - u) * (1 - v) + VolData(lby, lbx, i) * (1 - u) * v + VolData(rty, rtx, i) * u * (1 - v) + VolData(rby, rbx, i) * u * v;
                ...
                for p = 0 : XTNeighborPoints - 1
                    ...
                    % pixels are estimated by bilinear interpolation of
                    % four corner points near to it.
                    CurrentVal = VolData(yc, ltx, lty) * (1 - u) * (1 - v) + VolData(yc, lbx, lby) * (1 - u) * v + VolData(yc, rtx, rty) * u * (1 - v) + VolData(yc, rbx, rby) * u * v;
                ...
                for p = 0 : YTNeighborPoints - 1
                    ...
                    % pixels are estimated by bilinear interpolation of
                    % four corner points near to it.
                    CurrentVal = VolData(ltx, xc, lty) * (1 - u) * (1 - v) + VolData(lbx, xc, lby) * (1 - u) * v + VolData(rtx, xc, rty) * u * (1 - v) + VolData(rbx, xc, rby) * u * v;
