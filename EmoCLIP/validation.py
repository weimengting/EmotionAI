...
from models.downstream import DownstreamTask
from models.video_clip import VClip
import os
import argparse
import logging
...
def main(args):
    ...
    vc_cl = VClip(num_layers=2)
    model = DownstreamTask(clip_model=vc_cl, n_classes=num_classes).to(device=device, dtype=torch.float32)
    model_save_path = './saved_models/fold_' + str(args.index) + '_' + args.sign
    epoch = args.epoch

    dict_s = torch.load(os.path.join(model_save_path, 'trained_EmoClip_val_' + str(epoch) + '.pth'))
